...
import { PostsItemForMain } from '@/components/PostsListItem';
...
import { ArticleLayoutProps, Role } from '@/types/contentType';
...
function Article({ article }: ArticleLayoutProps) {
  return (
    <Card as='article'>
...
function SocialLink({ icon: Icon, href, ...props }: React.PropsWithChildren<SocialLinkProps>) {
  return (
    <Link className='group -m-1 p-1' href={href ? href : ''} {...props}>
          ...
          <li key={roleIndex} className='flex gap-4'>
            ...
            <dl className='flex flex-auto flex-wrap gap-x-2'>
              ...
              >
                ...
                <time dateTime={role.end.dateTime}>{role.end.label ?? role.end.dateTime}</time>
              </dd>
            </dl>
          </li>
        ))}
...
export default async function Home() {
  ...
  // const articles = a wait getAllArticles().slice(0, 4).map({ component, ...meta }) => meta
  // const articles = await getAllArticles();
  const posts = (await searchPosts(null, 4)).results;
      ...
      <Container className='mt-9'>
        <div className='max-w-2xl'>
          <h1 className='text-4xl font-bold tracking-tight text-zinc-800 dark:text-zinc-100 sm:text-5xl'>
            Software Developer, Founder and Father of one daughter.
          </h1>
          <p className='mt-6 text-base text-zinc-600 dark:text-zinc-400'>
            안녕하세요. 제 홈페이지에 오신 것을 환영 합니다. 별다른건 없고 Notion 기반 블로그 위주로 운영되는 사이트
            입니다. 이곳에서 잠시나마 즐겁거나, 영감을 얻거나, 평안한 기분을 느끼시면 좋을거 같네요. 감사합니다.
          </p>
      ...
      <Container className='mt-24 md:mt-28'>
        <div className='mx-auto grid max-w-xl grid-cols-1 gap-y-20 lg:max-w-none lg:grid-cols-2'>
          <div className='flex flex-col gap-16'>
            {posts.map(post => (
              <PostsItemForMain key={post.id} post={post} />
            ))}
          </div>
